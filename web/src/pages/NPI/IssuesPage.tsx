...
const severityColors: Record<string, string> = { '严重': 'red', '主要': 'orange', '次要': 'blue', '建议': 'green' };
const statusColors: Record<string, string> = { '待处理': 'red', '处理中': 'orange', '已解决': 'green', '已关闭': 'default' };
...
export default function IssuesPage() {
    ...
    { title: '负责人', dataIndex: 'assignee', key: 'assignee', width: 100 },
    {
      ...
      render: (_: any, r: any) => (
        <Space>
          ...
          <Button type="link" size="small" onClick={() => openEdit(r)}>处理</Button>
        </Space>
      ),
    },
  ];
  ...
  return (
    <Card title="问题跟踪" extra={<Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>提交问题</Button>}>
      <Table dataSource={data} columns={columns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} />

      <Modal title={editing ? '处理问题' : '提交问题'} open={modalOpen} onOk={submit} onCancel={() => setModalOpen(false)} width={640}>
        <Form form={form} layout="vertical">
          ...
          <Space style={{ width: '100%' }} size={16}>
            <Form.Item name="severity" label="严重程度" initialValue="主要" style={{ width: 180 }}>
              <Select options={['严重','主要','次要','建议'].map(s => ({ value: s, label: s }))} />
            </Form.Item>
            <Form.Item name="status" label="状态" initialValue="待处理" style={{ width: 180 }}>
              <Select options={['待处理','处理中','已解决','已关闭'].map(s => ({ value: s, label: s }))} />
            </Form.Item>
          </Space>
      ...
      <Modal title="问题详情" open={detailOpen} onCancel={() => setDetailOpen(false)} footer={null} width={520}>
        {detail && (
          <div>
            <p><strong>编号：</strong>{detail.issueCode}</p>
            <p><strong>标题：</strong>{detail.title}</p>
            <p><strong>描述：</strong>{detail.description || '-'}</p>
            <p><strong>严重程度：</strong><Tag color={severityColors[detail.severity]}>{detail.severity}</Tag></p>
            <p><strong>状态：</strong><Tag color={statusColors[detail.status]}>{detail.status}</Tag></p>
            <p><strong>负责人：</strong>{detail.assignee || '-'}</p>
            <p><strong>解决方案：</strong>{detail.solution || '-'}</p>
            <p><strong>解决时间：</strong>{detail.resolvedAt ? new Date(detail.resolvedAt).toLocaleDateString() : '-'}</p>
          </div>
        )}
      </Modal>
    </Card>
