...
export default function EfficiencyPage() {
  ...
  const loadEfficiency = async (orderId: string) => {
    ...
    setLoading(true);
    try {
      setEfficiency(await manufacturingApi.getEfficiencyByOrder(orderId));
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadWorkerStats = async () => {
    ...
    try {
      setWorkerStats(await manufacturingApi.getEfficiencyByWorker());
    } finally {
        ...
        {efficiency && (
          <div>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={6}>
                <Card size="small">
                  <Statistic title="计划总工时" value={efficiency.totalPlannedHours} suffix="h" />
                </Card>
              </Col>
              <Col span={6}>
                <Card size="small">
                  <Statistic title="实际总工时" value={efficiency.totalActualHours} suffix="h" />
                </Card>
              </Col>
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    valueStyle={{
                      color:
                        efficiency.overallEfficiency >= 90
                          ? 'green'
              ...
              <Col span={6}>
                <Card size="small">
                  <Statistic title="产品" value={efficiency.productName} />
                </Card>
              </Col>
            </Row>
            <Table
              dataSource={efficiency.details || []}
              rowKey="opSequence"
              size="small"
              columns={[
                ...
                { title: '实际工时', dataIndex: 'actualHours' },
                {
                  ...
                  render: (v: number) => (
                    <span
                      style={{
                        color: v >= 90 ? 'green' : v >= 70 ? 'orange' : 'red',
                        fontWeight: 'bold',
                      }}
                    >
                      {v}%
