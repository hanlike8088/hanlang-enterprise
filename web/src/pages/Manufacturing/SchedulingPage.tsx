...
export default function SchedulingPage() {
  ...
  const submit = async () => {
    ...
    if (editing) {
      await manufacturingApi.updatePlan(editing.id, v);
      message.success('排产计划已更新');
    } else {
      await manufacturingApi.createPlan(v);
  ...
  const remove = async (id: string) => {
    await manufacturingApi.deletePlan(id);
    message.success('已删除');
    fetch();
  };
  ...
  const viewDetail = async (r: any) => {
    try {
      setDetail(await manufacturingApi.getPlan(r.id));
    } catch {
      setDetail(r);
    }
  };
  ...
  const checkCapacity = async (planId: string) => {
    try {
      setCapacityAlerts(await manufacturingApi.checkCapacity(planId));
    } catch {
  ...
  const fetchCalendars = async () => {
    try {
      setCalendars(await manufacturingApi.getCalendars());
    } catch {}
  };
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card size="small">
            <Statistic title="排产计划" value={stats.planTotal || 0} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic title="进行中工单" value={stats.orderInProgress || 0} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic title="已完工" value={stats.orderCompleted || 0} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              valueStyle={{ color: stats.overdueCount > 0 ? 'red' : undefined }}
            />
          </Card>
        </Col>
      </Row>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="planPeriod" label="计划周期" rules={[{ required: true }]}>
          ...
          <Form.Item name="capacityHours" label="产能上限(小时)">
            <Input type="number" />
          </Form.Item>
      ...
      <Modal
        title={`排产明细 - ${detail?.planName || ''}`}
        open={!!detail}
        onCancel={() => {
          setDetail(null);
          setCapacityAlerts(null);
        }}
        width={900}
        footer={null}
      >
          ...
          <>
            <Space style={{ marginBottom: 12 }}>
              <Button
                ...
                onClick={() => {
                  itemForm.resetFields();
                  itemForm.setFieldsValue({ status: 'planned', resourceType: 'machine' });
                  setItemModal(true);
                }}
              >
                添加明细
              </Button>
              <Button icon={<WarningOutlined />} onClick={() => checkCapacity(detail.id)}>
                产能检查
              </Button>
            </Space>
            {capacityAlerts && (
              <Card size="small" style={{ marginBottom: 12, background: '#fff7e6' }}>
                {capacityAlerts.alerts?.length === 0 ? (
                  <span style={{ color: 'green' }}>产能正常</span>
                ) : (
                  capacityAlerts.alerts?.map((a: any, i: number) => (
                    <div key={i} style={{ color: 'red' }}>
                      ⚠ {a.workCenter}: {a.usedHours}h已超 {a.capacityHours}h上限 (超出{a.overload}
                      h)
                    </div>
                  ))
                )}
                  ...
                  render: (_: any, r: any) => (
                    <Space size="small">
                      <Button
                        ...
                        onClick={() => {
                          const sd =
                            prompt('新开始日期', r.startDate?.split('T')[0]) || r.startDate;
                      ...
                      <Popconfirm title="删除?" onConfirm={() => deleteItem(r.id)}>
      ...
      >
        <Form form={itemForm} layout="vertical">
          <Form.Item name="itemName" label="明细名称" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item name="resourceType" label="资源类型">
            <Select
              options={[
                ...
                { label: '人员', value: 'labor' },
              ]}
            />
          </Form.Item>
          <Form.Item name="resourceId" label="资源编号">
            <Input />
          </Form.Item>
          <Form.Item name="startDate" label="开始日期">
            <Input type="date" />
      ...
      >
        <Form form={calForm} layout="vertical" onFinish={upsertCal}>
          ...
          <Form.Item name="isWorkingDay" label="工作日?" initialValue={true}>
            <Select
              options={[
                { label: '是', value: true },
                { label: '否', value: false },
          ...
          <Form.Item name="capacityHours" label="产能小时">
            <Input type="number" />
          </Form.Item>
        ...
        <Card size="small" style={{ marginTop: 12 }}>
          ...
          {calendars.map((c: any) => (
            <div key={c.id}>
              {c.calendarDate?.split('T')[0]} {c.shift} {c.isWorkingDay ? '工作日' : '休息'}
            </div>
          ))}
        </Card>
      </Modal>
