...
const typeColor: Record<string, string> = {
  info: 'blue', warning: 'orange', error: 'red', success: 'green', announcement: 'purple',
};
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}><BellOutlined /> 通知中心</Title>
        <Space>
          <Button icon={<CheckOutlined />} onClick={handleMarkAllRead} disabled={!items.some(i => !i.isRead)}>
            All Read
          </Button>
          <Button icon={<DeleteOutlined />} onClick={handleCleanup} disabled={!items.some(i => i.isRead)}>
            Clear Read
          </Button>
        </Space>
      </div>
      ...
      <List
        ...
        renderItem={(item: Notification) => (
          <List.Item
            style={{ background: item.isRead ? 'transparent' : '#f6ffed', padding: '12px 16px', borderRadius: 6, marginBottom: 4 }}
            actions={[
              !item.isRead && <Button type="link" size="small" icon={<CheckOutlined />} onClick={() => handleMarkRead(item.id)}>Read</Button>,
