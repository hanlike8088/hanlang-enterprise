...
export default function MaterialsPage() {
  ...
  const handleDelete = (id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await erpApi.deleteMaterial(id);
          message.success('已删除');
          loadData();
        } catch {
          message.error('删除失败');
  ...
  const columns = [
    ...
    { title: '单位', dataIndex: 'unit', width: 60 },
    {
      ...
      dataIndex: 'stock',
      width: 80,
      render: (v) => <Tag color={v > 0 ? 'blue' : '#ccc'}>{v || 0}</Tag>,
    },
  ...
  return (
    <div>
      ...
      >
        <Form
          layout="vertical"
          initialValues={editRecord || { unit: 'Pcs', status: 'active', batchManaged: false }}
          onFinish={handleSave}
        >
          <Form.Item
            name="materialCode"
