...
export default function 回款管理Page() {
  ...
  const fetchRefs = async () => {
    try {
      const [custRes, recRes] = await Promise.all([crmApi.get("/crm/customers"), crmApi.get("/crm/reconciliations")]);
  ...
  const handleEdit = (record: Payment) => {
    setEditing(record);
    form.setFieldsValue({ ...record, paymentDate: record.paymentDate ? dayjs(record.paymentDate) : dayjs() });
    setModalOpen(true);
  };
  ...
  const handleDelete = async (id: string) => {
    try { await crmApi.delete(`/crm/payments/${id}`); message.success("已删除"); fetchData(); }
  ...
  const columns: ColumnsType<Payment> = [
    ...
    {
      title: "方式", dataIndex: "paymentMethod", width: 90,
      render: (v) => <Tag>{methodOptions.find(m => m.value === v)?.label || v}</Tag>,
    },
    { title: "回款日期", render: (_, r) => new Date(r.paymentDate).toLocaleDateString(), width: 110 },
    { title: "参考号", dataIndex: "referenceNo", width: 140, render: (v) => v || "-" },
    { title: "关联对账", render: (_, r) => r.reconciliation?.reconciliationCode || "-", width: 140 },
    {
      title: "操作", width: 140,
      render: (_, r) => (
        <Space>
  ...
  return (
    <Card title="客户回款" extra={
      <Space>
        <Input.Search placeholder="搜索回款" value={keyword} onChange={e => setKeyword(e.target.value)} onSearch={fetchData} style={{ width: 200 }}/>
        <Button icon={<ReloadOutlined />} onClick={fetchData} />
        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate}>记录回款</Button>
      </Space>
    }>
      <Table rowKey="id" columns={columns} dataSource={data} loading={loading} scroll={{ x: 1000 }} />
      <Modal title={editing ? "编辑回款" : "记录回款"} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={500}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="reconciliationId" label="关联对账单">
            <Select allowClear showSearch optionFilterProp="label" placeholder="选择对账单(可选)" options={reconciliations.map(r => ({ label: `${r.reconciliationCode} - ¥${r.totalAmount}`, value: r.id }))} />
          </Form.Item>
