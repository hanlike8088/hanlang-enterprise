...
export default function OQCPage() {
  ...
  const fetchOqc = async () => {
    setOqcLoading(true);
    try {
      setOqcData(await qualityApi.getOutgoings(oqcStatusFilter, oqcKeyword || undefined));
    } finally {
      setOqcLoading(false);
    }
  };
  useEffect(() => {
    if (activeTab === 'oqc') fetchOqc();
  }, [activeTab, oqcStatusFilter, oqcKeyword]);

  const openOqcCreate = () => {
    setEditingOqc(null);
    oqcForm.resetFields();
    oqcForm.setFieldsValue({ quantity: 0, unit: '个' });
    setOqcModalOpen(true);
  };
  const submitOqc = async () => {
    const v = await oqcForm.validateFields();
    if (editingOqc) {
      await qualityApi.updateOutgoing(editingOqc.id, v);
      message.success('已更新');
    } else {
      await qualityApi.createOutgoing(v);
      message.success('已创建');
    }
    setOqcModalOpen(false);
    fetchOqc();
  };
  const judgeOqc = async (r: any, result: string) => {
    await qualityApi.updateOutgoing(r.id, {
      result,
      status: result === '合格' ? '已完成' : '不合格',
      inspector: '质检员',
    });
    message.success(`判定: ${result}`);
    fetchOqc();
  };
  ...
  const fetchNcr = async () => {
    setNcrLoading(true);
    try {
      setNcrData(await qualityApi.getNcrs(ncrStatusFilter));
    } finally {
      setNcrLoading(false);
    }
  };
  useEffect(() => {
    if (activeTab === 'ncr') fetchNcr();
  }, [activeTab, ncrStatusFilter]);

  const openNcrCreate = () => {
    ncrForm.resetFields();
    ncrForm.setFieldsValue({ severity: '一般', source: 'OQC' });
    setNcrModalOpen(true);
  };
  const submitNcr = async () => {
    const v = await ncrForm.validateFields();
    await qualityApi.createNcr(v);
    message.success('NCR已创建');
    setNcrModalOpen(false);
    fetchNcr();
  };
  const openReviewNcr = (r: any) => {
    setReviewNcrId(r.id);
    reviewForm.resetFields();
    reviewForm.setFieldsValue({ status: '已评审', disposition: '返工' });
    setReviewNcrOpen(true);
  };
  const submitReviewNcr = async () => {
    const v = await reviewForm.validateFields();
    await qualityApi.reviewNcr(reviewNcrId, v);
    message.success('NCR已评审');
    setReviewNcrOpen(false);
    fetchNcr();
  };
  ...
  const fetchCapa = async () => {
    setCapaLoading(true);
    try {
      setCapaData(await qualityApi.getCapas(undefined, capaStatusFilter));
    } finally {
      setCapaLoading(false);
    }
  };
  useEffect(() => {
    if (activeTab === 'capa') fetchCapa();
  }, [activeTab, capaStatusFilter]);

  const openCapaCreate = () => {
    capaForm.resetFields();
    capaForm.setFieldsValue({ status: '待实施' });
    setCapaModalOpen(true);
  };
  const submitCapa = async () => {
    const v = await capaForm.validateFields();
    await qualityApi.createCapa(v);
  ...
  const oqcColumns = [
    ...
    { title: '单位', dataIndex: 'unit', width: 50 },
    {
      ...
      dataIndex: 'status',
      width: 80,
      render: (s: string) => <Tag color={oqcStatusColors[s]}>{s}</Tag>,
    },
    {
      title: '结果',
      dataIndex: 'result',
  ...
  const ncrColumns = [
    ...
    { title: '缺陷类型', dataIndex: 'defectType', width: 100 },
    {
      ...
      dataIndex: 'severity',
      width: 70,
      render: (s: string) => <Tag color={sevColors[s]}>{s}</Tag>,
    },
    { title: '数量', dataIndex: 'quantity', width: 60 },
    {
      title: '状态',
      dataIndex: 'status',
      width: 80,
      render: (s: string) => <Tag color={ncrStatusColors[s]}>{s}</Tag>,
    },
        ...
        <Space size="small">
          ...
          {r.status === '已评审' && !r.capa && (
            <Button
              ...
              icon={<ToolOutlined />}
              onClick={() => {
                capaForm.setFieldsValue({ ncrId: r.id });
                setCapaModalOpen(true);
              }}
            >
              创建CAPA
  ...
  const capaColumns = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      width: 80,
      render: (s: string) => <Tag color={capaStatusColors[s]}>{s}</Tag>,
    },
    {
      title: '操作',
      key: 'act',
  ...
  return (
    <Card title="出货检验 OQC & 不合格品 NCR & CAPA" bodyStyle={{ padding: 0 }}>
              ...
              <>
                <Space style={{ padding: '16px', marginBottom: 12 }}>
                  ...
                  <Select
                    ...
                    allowClear
                    style={{ width: 110 }}
                    onChange={setOqcStatusFilter}
                    options={['待检', '已完成', '不合格'].map((v) => ({ label: v, value: v }))}
                  />
                  <Button type="primary" icon={<PlusOutlined />} onClick={openOqcCreate}>
                    新增出货检验
                  </Button>
                </Space>
                <Table
              ...
              <>
                <Space style={{ padding: '16px', marginBottom: 12 }}>
                  <Select
                    ...
                    allowClear
                    style={{ width: 110 }}
                    onChange={setNcrStatusFilter}
                    options={['待评审', '已评审', '已处置', '已关闭'].map((v) => ({
                      label: v,
                      value: v,
                    }))}
                  />
                  <Button type="primary" icon={<PlusOutlined />} onClick={openNcrCreate}>
                    新增NCR
                  </Button>
                </Space>
                <Table
              ...
              <>
                <Space style={{ padding: '16px', marginBottom: 12 }}>
                  <Select
                    ...
                    allowClear
                    style={{ width: 110 }}
                    onChange={setCapaStatusFilter}
                    options={['待实施', '实施中', '已验证'].map((v) => ({ label: v, value: v }))}
                  />
                  <Button type="primary" icon={<PlusOutlined />} onClick={openCapaCreate}>
                    新增CAPA
                  </Button>
                </Space>
                <Table
      ...
      {/* OQC Modal */}
      <Modal
        title={editingOqc ? '编辑出货检验' : '新增出货检验'}
        open={oqcModalOpen}
        onOk={submitOqc}
        onCancel={() => setOqcModalOpen(false)}
      >
      ...
      >
        <Form form={ncrForm} layout="vertical">
          ...
          <Form.Item name="defectType" label="缺陷类型" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="severity" label="严重等级">
                <Select options={['一般', '严重', '致命'].map((v) => ({ label: v, value: v }))} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="quantity" label="不合格数量">
                <InputNumber style={{ width: '100%' }} min={0} />
              </Form.Item>
            </Col>
          </Row>
      ...
      <Modal
        title="NCR评审"
        open={reviewNcrOpen}
        onOk={submitReviewNcr}
        onCancel={() => setReviewNcrOpen(false)}
      >
        <Form form={reviewForm} layout="vertical">
          ...
          <Form.Item name="disposition" label="处置方式">
            <Select
              options={['返工', '报废', '让步接收', '退货'].map((v) => ({ label: v, value: v }))}
            />
          </Form.Item>
          <Form.Item name="reviewedBy" label="评审人">
            <Input />
          </Form.Item>
        </Form>
      </Modal>
      ...
      >
        <Form form={capaForm} layout="vertical">
          <Form.Item name="ncrId" label="关联NCR ID" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="rootCause" label="根本原因">
            <Input.TextArea rows={2} />
          </Form.Item>
          <Form.Item name="correctiveAction" label="纠正措施">
            <Input.TextArea rows={2} />
          </Form.Item>
