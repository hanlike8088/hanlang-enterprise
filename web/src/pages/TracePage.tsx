  ...
  const handleBatchTrace = async () => {
    ...
    try {
      ...
      const [tracesRes, fullRes] = await Promise.all([
        ...
        batchNo
          ? fetch(`/api/batch-trace/traces/full/${batchNo}`, {
              headers: { Authorization: `Bearer ${token}` },
            }).then((r) => r.json())
          : null,
      ]);
  ...
  return (
    <div>
      <Title level={4} style={{ marginBottom: 16 }}>
        Module Trace &amp; Batch Traceability
      </Title>
              ...
              <div>
                          ...
                          items={chain.nodes.map((node: any) => ({
                            ...
                            children: (
                              <div>
                                ...
                                <Text
                                  type="secondary"
                                  style={{ fontFamily: 'monospace', marginLeft: 8, fontSize: 11 }}
                                >
                                  {node.id?.slice(0, 12)}...
                                </Text>
                                {Object.entries(node.detail || {}).map(([k, v]) => (
                                  <Tag key={k} style={{ marginLeft: 4 }}>
                                    {k}: {String(v)}
                                  </Tag>
                                ))}
                              </div>
                ...
                {eventChain?.chains && (
                  <Card
                    title={
                      <>
                        <SwapOutlined /> Cross-Module Event Chains
                      </>
                    }
                    size="small"
                  >
              ...
              <div>
                {batchStats && (
                  <Row gutter={16} style={{ marginBottom: 16 }}>
                    ...
                    <Col span={6}>
                      <Card size="small">
                        <Text type="secondary">追溯记录</Text>
                        <Title level={5} style={{ margin: 0 }}>
                          {batchStats.traceCount}
                        </Title>
                      </Card>
                    </Col>
                    <Col span={6}>
                      <Card size="small">
                        <Text type="secondary">活跃批次</Text>
                        <Title level={5} style={{ margin: 0 }}>
                ...
                <Card size="small" style={{ marginBottom: 16 }}>
                  <Space wrap>
                    ...
                    <Button icon={<HistoryOutlined />} onClick={loadBatches}>
                      刷新批次列表
                    </Button>
                  </Space>
                </Card>
                          ...
                          items={fullTrace.forward.map((t: any) => ({
                            ...
                            children: (
                              <div>
                                ...
                                <Tag>
                                  {t.sourceType} {'->'} {t.targetType || 'END'}
                                </Tag>
                                <Text
                                  type="secondary"
                                  style={{ fontFamily: 'monospace', marginLeft: 4, fontSize: 11 }}
                                >
