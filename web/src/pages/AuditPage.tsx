      ...
      render: (_: any, record: any) => (
        <Space size="small">
          <Button
            ...
            size="small"
            icon={<EditOutlined />}
            onClick={() => openModal('finding', record)}
          />
      ...
      return (
        <Form form={form} layout="vertical">
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item name="planMonth" label="审核月份">
          ...
          <Form.Item name="leadAuditor" label="审核组长">
            <Input />
          </Form.Item>
          <Form.Item name="team" label="审核组成员">
            <Input.TextArea rows={2} placeholder="多人以逗号分隔" />
          </Form.Item>
          <Form.Item name="scope" label="审核范围">
    ...
    if (modalType === 'checklist')
      return (
        <Form form={form} layout="vertical">
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="checkMethod" label="审核方法" initialValue="现场检查">
                <Select
                  options={[
                    { label: '现场检查', value: '现场检查' },
                    { label: '文件审查', value: '文件审查' },
                    { label: '人员访谈', value: '人员访谈' },
                    { label: '记录抽查', value: '记录抽查' },
                  ]}
                />
              </Form.Item>
            </Col>
          ...
          <Form.Item name="evidence" label="客观证据">
            <Input.TextArea rows={2} />
          </Form.Item>
        </Form>
      );
    ...
    return (
      <Form form={form} layout="vertical">
        <Form.Item name="findingType" label="发现类型" initialValue="不符合项">
          <Select
            options={[
              { label: '不符合项', value: '不符合项' },
              { label: '观察项', value: '观察项' },
              { label: '改进机会', value: '改进机会' },
            ]}
          />
        </Form.Item>
        <Row gutter={16}>
        ...
        <Form.Item name="clause" label="对应条款">
          <Input placeholder="如 ISO 9001:2015 8.5.2" />
        </Form.Item>
        <Form.Item name="evidence" label="客观证据">
          <Input.TextArea rows={2} />
        </Form.Item>
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card size="small">
            <Statistic title="审核计划总数" value={stats?.totalPlans || 0} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic title="检查表总数" value={checklists.length} />
          </Card>
        </Col>
      </Row>
      ...
      <Modal
        title={
          editingItem
            ? `编辑${modalType === 'plan' ? '审核计划' : modalType === 'checklist' ? '检查项' : '发现项'}`
            : `新建${modalType === 'plan' ? '审核计划' : modalType === 'checklist' ? '检查项' : '发现项'}`
        }
        open={modalOpen}
        onOk={handleModalOk}
