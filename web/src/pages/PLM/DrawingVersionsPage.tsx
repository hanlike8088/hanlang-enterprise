...
import {
  Table, Button, Modal, Form, Input, Select, Tag, Space, message, Statistic, Row, Col, Card, Descriptions, Typography,
} from 'antd';
import {
  PlusOutlined, UploadOutlined, HistoryOutlined, EyeOutlined, DeleteOutlined, SwapOutlined, DiffOutlined,
...
export default function Drawing版本sPage() {
  ...
  useEffect(() => { load图纸管理(); plmApi.getProducts().then(set产品管理).catch(() => {}); }, [filterStatus]);
  ...
  const columns = [
    ...
    { title: 'Category', dataIndex: 'category', key: 'category', width: 100, render: (c: string) => c || '-' },
    { title: '版本', dataIndex: 'latest版本', key: 'latest版本', width: 100, render: (v: string) => v ? <Tag color="blue">{v}</Tag> : '-' },
    { title: 'Count', key: 'versionCount', width: 80, render: (_: any, r: any) => r.versions?.length || 0 },
    { title: 'Status', dataIndex: 'status', key: 'status', width: 90,
      render: (s: string) => { const st = STATUS_MAP[s] || { color: 'default', label: s }; return <Tag color={st.color}>{st.label}</Tag>; } },
    { title: 'Updated', dataIndex: 'updatedAt', key: 'updatedAt', width: 120, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
    { title: 'Actions', key: 'actions', width: 250,
      render: (_: any, record: any) => (
        <Space size="small">
          <Button size="small" icon={<EyeOutlined />} onClick={() => handleViewDetail(record.id)}>Detail</Button>
          <Button size="small" icon={<UploadOutlined />} onClick={() => { setSelectedDrawing(record); set版本Open(true); }}>New Ver</Button>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setCreateOpen(true); }}>New Drawing</Button>
      </div>

      <Table dataSource={drawings} columns={columns} rowKey="id" loading={loading} pagination={{ pageSize: 15 }} scroll={{ x: 1100 }} />

      <Modal title="New Drawing" open={createOpen} onCancel={() => { setCreateOpen(false); form.resetFields(); }} footer={null} width={550}>
      ...
      <Modal title="Drawing Detail" open={detailOpen} onCancel={() => setDetailOpen(false)} footer={<Button onClick={() => setDetailOpen(false)}>Close</Button>} width={700}>
        {selectedDrawing && (
          <div>
            ...
            <Table
              ...
              pagination={false}
              columns={[
                { title: '版本', dataIndex: 'version', width: 80, render: (v: string, r: any) => r.isLatest ? <Tag color="blue">{v}</Tag> : v },
                { title: 'Change', dataIndex: 'changeNote', render: (n: string) => n || '-' },
                { title: 'File', dataIndex: 'fileName', width: 150, render: (n: string) => n || '-' },
                { title: 'Type', dataIndex: 'docType', width: 70 },
                { title: 'Uploader', dataIndex: 'uploadBy', width: 90 },
                { title: 'Compare', key: 'verActions', width: 80, render: (_: any, vr: any) => vr.isLatest ? null : <Button size="small" icon={<DiffOutlined />} onClick={() => { if (selectedDrawing) { const lv = versions.find((v:any) => v.isLatest); if (lv) handleCompare版本s(selectedDrawing.id, vr.id, lv.id); } }}>Compare</Button> },
                { title: 'Time', dataIndex: 'createdAt', width: 120, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
              ]}
            />
          </div>
        )}
      ...
      <Modal title="版本 Compare" open={compareOpen} onCancel={() => setCompareOpen(false)} footer={<Button onClick={() => setCompareOpen(false)}>Close</Button>} width={600}>
        {compareData && (<>
          <Row gutter={16}>
            <Col span={12}><Card size="small" title={'Ver ' + compareData.version1?.version}><p>Uploader: {compareData.version1?.uploadBy}</p><p>Time: {compareData.version1?.createdAt ? dayjs(compareData.version1.createdAt).format('YYYY-MM-DD HH:mm') : '-'}</p><p>File: {compareData.version1?.fileName || '-'}</p><p>Note: {compareData.version1?.changeNote || '-'}</p></Card></Col>
            <Col span={12}><Card size="small" title={'Ver ' + compareData.version2?.version}><p>Uploader: {compareData.version2?.uploadBy}</p><p>Time: {compareData.version2?.createdAt ? dayjs(compareData.version2.createdAt).format('YYYY-MM-DD HH:mm') : '-'}</p><p>File: {compareData.version2?.fileName || '-'}</p><p>Note: {compareData.version2?.changeNote || '-'}</p></Card></Col>
          </Row>
        </>)}
      </Modal>
    </div>
