...
export default function 专利管理Page() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 110,
      render: (t: string) =>
        t ? <Tag color={patentTypeColors[t] || 'default'}>{t}</Tag> : <Tag>未分类</Tag>,
    },
    {
      title: '关联产品',
      dataIndex: 'product',
    ...
    },
    {
      ...
      render: (d: string) => {
        ...
        return (
          <span style={{ color: daysLeft <= 90 ? '#ff4d4f' : undefined }}>
  ...
  return (
    <>
      ...
      <Modal
        ...
        onCancel={() => set即将到期ModalOpen(false)}
        footer={null}
        width={700}
      >
        <Table
          dataSource={expiring专利管理}
          rowKey="id"
          size="small"
          pagination={false}
          columns={[
            { title: '专利名称', dataIndex: 'docName', ellipsis: true },
            {
              ...
              render: (d: string) => {
                ...
                return (
                  <Tag color={daysLeft <= 30 ? 'red' : 'orange'}>
                    {dayjs(d).format('YYYY-MM-DD')} ({daysLeft}天)
                  </Tag>
                );
              },
            },
