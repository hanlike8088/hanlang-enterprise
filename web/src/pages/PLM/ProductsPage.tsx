...
import { Table, Button, Modal, Form, Input, Select, Space, Card, Tag, message, Popconfirm, Row, Col, InputNumber } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, PictureOutlined, SettingOutlined, FileProtectOutlined } from '@ant-design/icons';
...
export default function 产品管理Page() {
  ...
  const submit = async () => {
    ...
    const specs: any = {};
    specFields.forEach(f => { if (values[f.key] !== undefined && values[f.key] !== '') specs[f.key] = values[f.key]; });
    delete values.voltage; delete values.power; delete values.speed; delete values.torque;
    const payload = { ...values, specifications: Object.keys(specs).length > 0 ? JSON.stringify(specs) : undefined };
    if (editing) {
  ...
  const editBom = (bom: any) => {
    setBomEditing(bom);
    bomForm.setFieldsValue({ materialId: bom.materialId, quantity: bom.quantity, unit: bom.unit, version: bom.version });
  };
  ...
  const productColumns = [
    ...
    { title: '规格参数', key: 'specs', width: 200, render: (_: any, r: any) => renderSpecs(r.specifications) },
    { title: '类别', dataIndex: 'category', key: 'category', width: 130, render: (v: string) => v || '-' },
    { title: '型号', dataIndex: 'modelNo', key: 'modelNo', width: 110, render: (v: string) => v || '-' },
    { title: '状态', dataIndex: 'status', key: 'status', width: 90, render: (s: string) => <Tag color={statusColors[s] || 'default'}>{s}</Tag> },
    { title: 'BOM', key: 'bomCount', width: 60, render: (_: any, r: any) => r.boms?.length || 0 },
    {
      ...
      render: (_: any, r: any) => (
        <Space>
          ...
          <Button type="link" size="small" icon={<PictureOutlined />} onClick={() => { setDrawingProduct(r); drawingApi.getDrawings(r.id).then(set图纸管理); set图纸管理ModalOpen(true); }}>图纸</Button>
          <Button type="link" size="small" icon={<FileProtectOutlined />} onClick={() => { set专利Product(r); plmApi.getDocuments(r.id, '专利').then(set专利管理); set专利管理ModalOpen(true); }}>专利</Button>
  ...
  return (
    <>
      <Card title="PLM 产品管理" extra={<Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>新建产品</Button>}>
        <Row gutter={12} style={{ marginBottom: 16 }}>
          <Col flex="auto">
            <Input placeholder="搜索产品名称或编码" value={searchText} onChange={e => setSearchText(e.target.value)} allowClear />
          </Col>
          <Col>
            <Select placeholder="类别" value={filterCategory} onChange={setFilterCategory} allowClear style={{ width: 140 }} options={allCategories.map(c => ({ value: c, label: c }))} />
          </Col>
          <Col>
        ...
        <Table dataSource={filtered产品管理} columns={productColumns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} />
      </Card>

      <Modal title={editing ? '编辑产品' : '新建产品'} open={modalOpen} onOk={submit} onCancel={() => setModalOpen(false)} width={680}>
        <Form form={form} layout="vertical">
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="category" label="类别">
                <Select options={['循环扇电机','罩极电机','无刷直流电机','静音电机','其他'].map(s => ({ value: s, label: s }))} />
              </Form.Item>
            </Col>
            <Col span={12}>
          ...
          <Form.Item name="status" label="状态" initialValue="开发中">
            <Select options={['开发中','试产中','已发布','已停产'].map(s => ({ value: s, label: s }))} />
          </Form.Item>
      ...
      <Modal title={'BOM 管理 - ' + (bomProduct?.productName || '')} open={bomModalOpen} onCancel={() => { setBomModalOpen(false); setBomEditing(null); }} footer={null} width={700}>
        <Card size="small" style={{ marginBottom: 16 }} title={bomEditing ? '编辑 BOM' : '添加 BOM'}>
          <Form form={bomForm} layout="inline" style={{ flexWrap: 'wrap', gap: 8 }}>
            <Form.Item name="materialId" label="物料" rules={[{ required: true }]} style={{ minWidth: 200 }}>
              <Select options={materials.map(m => ({ value: m.id, label: m.materialCode + ' - ' + m.materialName }))} placeholder="选择物料" />
            </Form.Item>
            ...
            <Form.Item>
              <Space>
                <Button type="primary" onClick={submitBom}>{bomEditing ? '保存' : '添加'}</Button>
                {bomEditing && <Button onClick={cancelBomEdit}>取消</Button>}
              </Space>
            </Form.Item>
          </Form>
        </Card>
        <Table dataSource={bomList} columns={bomColumns} rowKey="id" loading={bomLoading} size="small" pagination={false} locale={{ emptyText: '尚无 BOM 记录，请在上方添加' }} />
      </Modal>

      <Modal title={'图纸 - ' + (drawingProduct?.productName || '')} open={drawingsModalOpen} onCancel={() => set图纸管理ModalOpen(false)} footer={null} width={700}>
        <Table dataSource={drawings} rowKey="id" size="small" pagination={false}
          columns={[
            ...
            { title: '版本', dataIndex: 'latest版本', width: 80, render: (v: string) => <Tag color="blue">{v}</Tag> },
            { title: 'Category', dataIndex: 'category', width: 100, render: (c: string) => c || '-' },
            { title: 'Updated', dataIndex: 'updatedAt', width: 120, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
          ]}
        />
      </Modal>

      <Modal title={'专利 - ' + (patentProduct?.productName || '')} open={patentsModalOpen} onCancel={() => set专利管理ModalOpen(false)} footer={null} width={700}>
        <Table dataSource={patents} rowKey="id" size="small" pagination={false} locale={{ emptyText: '暂无关联专利' }}
          columns={[
            { title: '专利编码', dataIndex: 'docCode', width: 120 },
            { title: '专利名称', dataIndex: 'docName', ellipsis: true },
            { title: '类型', dataIndex: 'patentType', width: 100, render: (t) => t ? <Tag color="blue">{t}</Tag> : '-' },
            { title: '到期日', dataIndex: 'expirationDate', width: 110, render: (d) => d ? dayjs(d).format('YYYY-MM-DD') : '-' },
          ]}
        />
      </Modal>
    </>
