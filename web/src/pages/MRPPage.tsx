...
export default function MRPPage() {
  ...
  const itemColumns = [
    ...
    },
    {
      ...
      render: (v: number) => (
        <span
          style={{ color: v > 0 ? '#ff4d4f' : '#52c41a', fontWeight: v > 0 ? 'bold' : 'normal' }}
        >
          {v.toFixed(2)}
        </span>
      ),
    ...
    },
    {
      ...
      width: 90,
      render: (s: string) =>
        s === 'open' ? <Tag color="red">待采购</Tag> : <Tag color="green">库存充足</Tag>,
    },
  ...
  const runColumns = [
    ...
    },
    {
      ...
      width: 90,
      render: (s: string) => (
        <Tag color={s === 'completed' ? 'green' : 'processing'}>
          {s === 'completed' ? '已完成' : '运算中'}
        </Tag>
      ),
    },
    ...
    },
    {
      ...
      width: 100,
      render: (v: number) => (
        <span style={{ color: v > 0 ? '#ff4d4f' : '#52c41a' }}>{v.toFixed(2)}</span>
      ),
    },
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              title="待处理物料"
              value={selectedRun?.items?.filter((i: any) => i.status === 'open').length || 0}
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              title="总缺口数量"
              value={selectedRun?.totalShortage || 0}
              precision={2}
              valueStyle={{ color: '#ff4d4f' }}
            />
          </Card>
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              title="最近运算"
              value={runs.length > 0 ? new Date(runs[0].createdAt).toLocaleDateString() : '-'}
            />
          </Card>
        </Col>
      </Row>
      ...
      <Card title="运算历史" size="small" style={{ marginBottom: 16 }}>
        <Table
          ...
          onRow={(record) => ({
            ...
            style: {
              cursor: 'pointer',
              background: selectedRun?.id === record.id ? '#e6f7ff' : undefined,
            },
          })}
