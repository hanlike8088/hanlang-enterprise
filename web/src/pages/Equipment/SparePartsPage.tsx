...
export default function SparePartsPage() {
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      setData(await equipmentApi.getSpareParts(keyword || undefined));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchWarnings = async () => {
    try {
      const [w, s] = await Promise.all([
        equipmentApi.getSparePartWarnings(),
        equipmentApi.getPurchaseSuggestions(),
      ]);
      setWarnings(w);
      setSuggestions(s);
    } catch {}
  ...
  const fetchRecords = async () => {
    try {
      setRecords(await equipmentApi.getSparePartRecords());
    } catch {}
  };
  ...
  const submit = async () => {
    ...
    if (editing) {
      await equipmentApi.updateSparePart(editing.id, v);
      message.success('已更新');
    } else {
      await equipmentApi.createSparePart(v);
      message.success('已创建');
    }
    setModalOpen(false);
    fetch();
  ...
  const remove = async (id: string) => {
    await equipmentApi.deleteSparePart(id);
    message.success('已删除');
    fetch();
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, r: any) => (
        <Space size="small">
          <Button type="link" size="small" onClick={() => openStockIn(r)}>
            入库
          </Button>
          <Button type="link" size="small" onClick={() => openStockOut(r)}>
            领用
          </Button>
  ...
  return (
    <Card title="备品备件管理" bodyStyle={{ padding: 0 }}>
          ...
          },
          {
            ...
            children: (
              <Table
                ...
                style={{ padding: 16 }}
                columns={[
                  { title: '备件', render: (_: any, r: any) => r.part?.partName || '-' },
                  {
                    title: '类型',
                    dataIndex: 'type',
                    width: 60,
                    render: (t: string) => <Tag color={t === '入库' ? 'green' : 'orange'}>{t}</Tag>,
                  },
                  { title: '数量', dataIndex: 'quantity', width: 60 },
                  { title: '变更前', dataIndex: 'beforeQty', width: 60 },
                  { title: '变更后', dataIndex: 'afterQty', width: 60 },
                  { title: '操作人', dataIndex: 'operator', width: 80 },
                  { title: '参考', dataIndex: 'reference', width: 100 },
                  {
                    title: '时间',
                    dataIndex: 'createdAt',
                    width: 150,
      ...
      <Modal
        ...
        open={inModal}
        onOk={submitStockIn}
        onCancel={() => setInModal(false)}
      >
        <Form form={inForm} layout="vertical">
          <p>
            备件：<strong>{selectedPart?.partName}</strong> (当前库存: {selectedPart?.currentStock})
          </p>
      ...
      >
        <Form form={outForm} layout="vertical">
          ...
          <Form.Item name="quantity" label="领用数量" rules={[{ required: true }]}>
            <InputNumber style={{ width: '100%' }} min={1} max={selectedPart?.currentStock || 0} />
          </Form.Item>
          <Form.Item name="reference" label="关联单据">
            <Input placeholder="如维修工单号" />
          </Form.Item>
