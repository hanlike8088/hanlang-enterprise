...
export default function RepairPage() {
  ...
  const fetch = async () => {
    ...
    try {
      const [reqs, eqs, st] = await Promise.all([
        equipmentApi.getRepairRequests(),
        equipmentApi.getEquipments(),
        equipmentApi.getRepairStats(),
      ]);
      setRequests(reqs);
      setEquipments(eqs);
      setStats(st);
  ...
  const startRepair = async (woId: string) => {
    await equipmentApi.startRepair(woId);
    message.success('维修已开始');
    fetch();
  };
  const completeRepair = async (woId: string) => {
    const v = {
      result: prompt('维修结果') || '已完成',
      partsUsed: prompt('更换备件（可选）') || undefined,
    };
    await equipmentApi.completeRepair(woId, v);
    message.success('维修完成，待验收');
    fetch();
  };
  const verifyRepair = async (woId: string) => {
    await equipmentApi.verifyRepair(woId, {
      verifiedBy: localStorage.getItem('user')
  ...
  const viewDetail = async (r: any) => {
    try {
      setDetail(await equipmentApi.getRepairRequest(r.id));
    } catch {}
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      width: 80,
      render: (s: string) => <Tag color={reqStatusColors[s]}>{s}</Tag>,
    },
    ...
    { title: '时间', dataIndex: 'reportedAt', width: 110, render: (v: string) => v?.split('T')[0] },
    {
      ...
      render: (_: any, r: any) => (
        <Space size="small">
          ...
          {r.status === '待派工' && (
            <Button type="link" size="small" onClick={() => openDispatch(r)}>
              派工
            </Button>
          )}
          {r.workOrder && r.workOrder.status === '待维修' && (
            <Button
              type="link"
              size="small"
              icon={<PlayCircleOutlined />}
              onClick={() => startRepair(r.workOrder.id)}
            >
              开始
            </Button>
          )}
          {r.workOrder && r.workOrder.status === '维修中' && (
            <Button
              type="link"
              size="small"
              icon={<StopOutlined />}
              onClick={() => completeRepair(r.workOrder.id)}
            >
              完成
            </Button>
          )}
          {r.workOrder && r.workOrder.status === '待验收' && (
            <Button
              type="link"
              size="small"
              icon={<CheckCircleOutlined />}
              onClick={() => verifyRepair(r.workOrder.id)}
            >
              验收
            </Button>
          )}
    ...
    <Card title="设备维修管理" bodyStyle={{ padding: 0 }}>
            ...
            children: (
              <div style={{ padding: 16 }}>
                <Row gutter={16} style={{ marginBottom: 12 }}>
                  <Col span={6}>
                    <Statistic title="维修总数" value={stats.totalRepairs || 0} />
                  </Col>
                  <Col span={6}>
                    <Statistic title="MTTR" value={stats.mttr || '-'} />
                  </Col>
                  <Col span={6}>
                    <Statistic title="MTBF" value={stats.mtbf || '-'} />
                  </Col>
                  <Col span={6}>
                    <Statistic title="总工时(h)" value={stats.totalRepairHours || 0} />
                  </Col>
                </Row>
                <Button
                  type="primary"
                    ...
                    <div style={{ padding: 16 }}>
                      <Descriptions bordered size="small" column={2}>
                        ...
                        <Descriptions.Item label="严重程度">
                          <Tag color={sevColors[detail.severity]}>{detail.severity}</Tag>
                        </Descriptions.Item>
                        <Descriptions.Item label="状态">
                          <Tag color={reqStatusColors[detail.status]}>{detail.status}</Tag>
                        </Descriptions.Item>
                        <Descriptions.Item label="报修人">{detail.reporter}</Descriptions.Item>
                        <Descriptions.Item label="报修时间">
                          {detail.reportedAt?.split('T')[0]}
                        </Descriptions.Item>
                        {detail.workOrder && (
                          <>
                            <Descriptions.Item label="工单号">
                              {detail.workOrder.orderCode}
                            </Descriptions.Item>
                            <Descriptions.Item label="负责人">
                              {detail.workOrder.assignedTo || '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="维修方法">
                              {detail.workOrder.repairMethod || '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="更换备件">
                              {detail.workOrder.partsUsed || '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="开始时间">
                              {detail.workOrder.startTime
                                ? dayjs(detail.workOrder.startTime).format('MM-DD HH:mm')
                                : '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="结束时间">
                              {detail.workOrder.endTime
                                ? dayjs(detail.workOrder.endTime).format('MM-DD HH:mm')
                                : '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="维修结果">
                              {detail.workOrder.result || '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="验收人">
                              {detail.workOrder.verifiedBy || '-'}
                            </Descriptions.Item>
                          </>
                        )}
                      </Descriptions>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      title: 'MTTR(h)',
                      render: (_: any, r: any) =>
                        r.count > 0 ? (r.totalHours / r.count).toFixed(1) : '-',
                    },
                  ]}
                />
              </div>
      ...
      >
        <Form form={dispatchForm} layout="vertical">
          <Form.Item name="assignedTo" label="指派维修人员" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="repairMethod" label="维修方案">
