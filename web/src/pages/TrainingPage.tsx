...
export default function TrainingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      switch (modalType) {
        case 'course':
          url = editingId ? `/api/training/courses/${editingId}` : '/api/training/courses';
          break;
        case 'record':
        ...
        case 'qualification':
          url = editingId
            ? `/api/training/qualifications/${editingId}`
            : '/api/training/qualifications';
          break;
        case 'skill':
          url = editingId ? `/api/training/skills/${editingId}` : '/api/training/skills';
          break;
        case 'plan':
          url = editingId ? `/api/training/plans/${editingId}` : '/api/training/plans';
          break;
      }
  ...
  const handleDelete = async (type: string, id: string) => {
    try {
      ...
      switch (type) {
        case 'course':
          url = `/api/training/courses/${id}`;
          break;
        case 'qualification':
          url = `/api/training/qualifications/${id}`;
          break;
        case 'skill':
          url = `/api/training/skills/${id}`;
          break;
        case 'plan':
      ...
      render: (_: any, r: any) => (
        <Space>
          ...
          <Button
            ...
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDelete('course', r.id)}
          />
        </Space>
      ),
    },
  ...
  const recordColumns = [
    { title: '\u8bb0\u5f55\u7f16\u53f7', dataIndex: 'recordCode', key: 'rc', width: 140 },
    {
      ...
      key: 'cn2',
      width: 150,
      render: (v: string, r: any) => v || r.courseName,
    },
    {
      title: '\u5458\u5de5',
      ...
      key: 'en',
      width: 100,
      render: (v: string, r: any) => v || r.employeeName,
    },
    {
      title: '\u57f9\u8bad\u65e5\u671f',
  ...
  const qualColumns = [
    ...
    },
    {
      ...
      render: (v: string) => (
        <Badge
          status={v === 'valid' ? 'success' : v === 'expired' ? 'error' : 'default'}
          text={v === 'valid' ? '\u6709\u6548' : v === 'expired' ? '\u8fc7\u671f' : v}
        />
      ),
    },
    {
      ...
      render: (_: any, r: any) => (
        <Space>
          <Button
            size="small"
            icon={<EditOutlined />}
            onClick={() => openModal('qualification', r)}
          />
          <Button
            size="small"
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDelete('qualification', r.id)}
          />
        </Space>
      ),
    },
      ...
      render: (_: any, r: any) => (
        <Space>
          ...
          <Button
            ...
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDelete('skill', r.id)}
          />
        </Space>
      ),
    },
      ...
      render: (_: any, r: any) => (
        <Space>
          ...
          <Button
            ...
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDelete('plan', r.id)}
          />
        </Space>
      ),
    },
  ...
  const renderModal = () => {
    ...
    return (
      <Modal
        title={`${editingId ? '\u7f16\u8f91' : '\u65b0\u589e'}${titleMap[modalType]}`}
        open={modalOpen}
        onOk={handleSubmit}
        ...
        <Form form={form} layout="vertical">
          {modalType === 'course' && (
            <>
              ...
              <Form.Item name="courseType" label={'\u7c7b\u578b'} initialValue="internal">
                <Select
                  options={[
          ...
          {modalType === 'record' && (
            <>
              ...
              <Form.Item name="trainingDate" label={'\u57f9\u8bad\u65e5\u671f'}>
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item name="hours" label={'\u8bfe\u65f6(h)'}>
                <InputNumber min={0} style={{ width: '100%' }} />
              </Form.Item>
              ...
              <Form.Item name="evaluator" label={'\u8003\u6838\u4eba'}>
                <Input />
              </Form.Item>
              <Form.Item name="certificateNo" label={'\u8bc1\u4e66\u7f16\u53f7'}>
                <Input />
              </Form.Item>
              <Form.Item name="expiryDate" label={'\u8bc1\u4e66\u5230\u671f\u65e5'}>
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
          ...
          {modalType === 'qualification' && (
            <>
              ...
              >
                <Select
                  options={[
                    ...
                    { value: 'license', label: '\u64cd\u4f5c\u8bb8\u53ef\u8bc1' },
                    { value: 'degree', label: '\u5b66\u5386\u5b66\u4f4d' },
              ...
              <Form.Item name="issueDate" label={'\u9881\u53d1\u65e5\u671f'}>
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
          ...
          {modalType === 'skill' && (
            <>
              ...
              <Form.Item name="proficiency" label={'\u719f\u7ec3\u5ea6'} initialValue="L1">
                <Select
                  options={[
                    { value: 'L1', label: 'L1 - \u5165\u95e8' },
                    { value: 'L2', label: 'L2 - \u57fa\u7840' },
                    { value: 'L3', label: 'L3 - \u719f\u7ec3' },
                    { value: 'L4', label: 'L4 - \u7cbe\u901a' },
                    { value: 'L5', label: 'L5 - \u4e13\u5bb6' },
                  ]}
                />
              </Form.Item>
              <Form.Item name="assessedBy" label={'\u8bc4\u4f30\u4eba'}>
                <Input />
              </Form.Item>
              <Form.Item name="assessedDate" label={'\u8bc4\u4f30\u65e5\u671f'}>
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item name="remark" label={'\u5907\u6ce8'}>
                <Input.TextArea rows={2} />
          ...
          {modalType === 'plan' && (
            <>
              ...
              <Form.Item name="planYear" label={'\u5e74\u5ea6'} rules={[{ required: true }]}>
                <InputNumber min={2020} style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item name="targetGroup" label={'\u76ee\u6807\u7fa4\u4f53'}>
                <Input />
              </Form.Item>
              <Form.Item name="targetCount" label={'\u76ee\u6807\u4eba\u6570'}>
                <InputNumber min={0} style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item name="completedCount" label={'\u5b8c\u6210\u4eba\u6570'}>
                <InputNumber min={0} style={{ width: '100%' }} />
              </Form.Item>
  ...
  return (
    <div>
      <Title level={4} style={{ marginBottom: 16 }}>
        <SafetyCertificateOutlined /> {'\u57f9\u8bad\u7ba1\u7406'}
      </Title>
        ...
        <Row gutter={16} style={{ marginBottom: 16 }}>
          ...
          <Col span={4}>
            <Card size="small">
              ...
              <Title
                level={5}
                style={{ margin: 0, color: stats.expiringQualCount > 0 ? '#ff4d4f' : undefined }}
              >
      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            label: (
              <>
                <SafetyCertificateOutlined /> {'\u8d44\u8d28\u8ba4\u8bc1'}
              </>
            ),
            children: (
              <>
                {stats?.expiringQualifications?.length > 0 && (
                  <Card size="small" style={{ marginBottom: 12, borderColor: '#ff4d4f' }}>
                    <Space>
                      ...
                      <Text type="danger">
                        {'\u5373\u5c06\u8fc7\u671f\u8d44\u8d28'} ({stats.expiringQualCount}
                        {'\u9879'})
                      </Text>
                    </Space>
                    <Table
                      ...
                      columns={[
                        ...
                        { title: '\u5458\u5de5', dataIndex: ['employee', 'name'], key: 'en' },
                        {
                          ...
                          key: 'ed',
                          render: (v: string) => (
                            <Text type="danger">{dayjs(v).format('YYYY-MM-DD')}</Text>
                          ),
                        },
